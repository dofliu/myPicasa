...
from PyQt5.QtWidgets import (
    ...
    QMessageBox, QDialog, QScrollArea, QAction, QMenuBar
)
...
class ImageTool(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("icon.png")) # 假設有一個icon.png檔案
        self.resize(800, 600)
        self.image = None # 新增屬性
        self._initUI()
        self.create_actions()
        self.create_menus()
    ...
    def _initUI(self):
        ...
        self.filesList.itemClicked.connect(self.previewImage) # 新增點擊事件
        ...
        self.image_label.setAlignment(Qt.AlignCenter) # 置中顯示
    ...
    def open_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "打開圖片", "", "Images (*.png *.xpm *.jpg *.jpeg *.bmp *.gif)")
        if file_path:
            try:
                self.image = Image.open(file_path)
                self.show_image()
            except Exception as e:
                QMessageBox.critical(self, "錯誤", f"無法打開圖片: {e}")
    ...
    def show_image(self):
        ...
        try:
            ...
            self.image_label.setScaledContents(True) # 允許圖片縮放

        except Exception as e:
            QMessageBox.critical(self, "錯誤", f"無法顯示圖片: {e}")
    ...
    def previewImage(self, item): # 新增點擊事件處理函式
        try:
            image_path = item.text()
            self.image = Image.open(image_path)
            self.show_image()
        except Exception as e:
             QMessageBox.critical(self, "錯誤", f"無法預覽圖片: {e}")
